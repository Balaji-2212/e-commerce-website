...
if (!sessionStorage.getItem('loggedInPhone') && !window.location.pathname.includes('login.html') && !window.location.pathname.includes('register.html')) {
  window.location.href = 'login.html';
}

// ── STATE ──────────────────────────────────────────────────────
if (!localStorage.getItem('leaves_cleared_v2')) {
  localStorage.removeItem('staffLeaves');
  localStorage.removeItem('STAFF_DATA_PERSIST');
  localStorage.setItem('leaves_cleared_v2', 'true');
}

// Clear cached appointments (one-time, to remove previously stored mock appointments)
if (!localStorage.getItem('apts_cleared_v1')) {
  localStorage.removeItem('STAFF_DATA_PERSIST');
  localStorage.removeItem('aptStates');
  localStorage.setItem('apts_cleared_v1', 'true');
}
...
if (typeof STAFF_DATA !== 'undefined') {
  ...
  const loggedInPhone = sessionStorage.getItem('loggedInPhone');
  ...
  if (matchedKey) {
    ...
    if (newKey && !STAFF_DATA[newKey]) {
      const regRole = sessionStorage.getItem('loggedInRole') || "Senior Stylist";
      const regPhone = sessionStorage.getItem('loggedInPhone') || "";
      const regEmail = sessionStorage.getItem('loggedInEmail') || "";
      const regSpecialties = JSON.parse(sessionStorage.getItem('loggedInSpecialties') || '[]');
...
// Appointments are managed via the admin portal — do not auto-generate
if (STAFF_DATA[currentStaff] && !STAFF_DATA[currentStaff].appointments) {
  STAFF_DATA[currentStaff].appointments = [];
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  setInterval(() => {
    // Skip refresh if any modal is currently open to keep UI stable
...
function updateLiveClock() {
  ...
  // Auto-refresh only specific dynamic fields when the minute changes (no full rebuild)
  ...
  if (window.lastCheckedMinute !== currentMin) {
    ...
    if (currentStaff) {
      const s = STAFF_DATA[currentStaff];
      if (s) {
        // Only update greeting text (time-sensitive)
        const displayName = sessionStorage.getItem('loggedInUser') || s.name;
        const greetEl = document.getElementById("greeting");
        if (greetEl) {
          const h = now.getHours();
          const greet = h < 12 ? "Good morning" : h < 17 ? "Good afternoon" : "Good evening";
          greetEl.textContent = greet + ", " + displayName.split(" ")[0] + " ✨";
        }

        // Only update serving/next client in the profile panel (lightweight)
        const inProgressApt = s.appointments && s.appointments.find(a => (aptStates[a.id] || a.status) === 'in-progress');
        const upcomingApts = s.appointments ? s.appointments.filter(a => (aptStates[a.id] || a.status) === 'upcoming') : [];
        const nextApt = upcomingApts.length > 0 ? upcomingApts[0] : null;

        s.currentClient = inProgressApt ? inProgressApt.client : null;
        s.nextClient = nextApt ? nextApt.client : null;
        s.nextTime = nextApt ? nextApt.time : null;

        // Update profile panel serving/next fields only
        if (s.currentClient) {
          show("pCurrentRow");
        ...
        // Refresh timeline and dashboard stats without full page rebuild
        if (typeof buildDashboard === 'function') buildDashboard(s);
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.services && data.services.length > 0) {
        serviceCatalog = data.services.map(s => {
          if (data.serviceTypes?.indoor?.includes(s.id)) s.type = 'indoor';
          if (data.serviceTypes?.outdoor?.includes(s.id)) s.type = 'outdoor';
          return s;
        });
      }
    ...
    console.warn("Running in local mode or db.json missing, using fallback data.");
  }

  // Populate fallback clients if db.json fetch failed
  if (!allClients || allClients.length === 0) {
    if (typeof FALLBACK_CLIENTS !== 'undefined') {
      allClients = FALLBACK_CLIENTS;
    }
  }
...
async function syncLiveAppointments() {
  try {
    ...
    // Build a lookup: for each STAFF_DATA key, collect all IDs that could match a booking staffId
    // A booking's staffId field is an ARRAY of id strings like ["STF-VMFZUI", "st1"]
    const staffIdAliases = {}; // key => Set of known IDs
    const loggedInUniqueId = sessionStorage.getItem('loggedInUniqueId') || '';

    Object.keys(STAFF_DATA).forEach(key => {
      const s = STAFF_DATA[key];
      const aliases = new Set();
      aliases.add(key);                               // local key, e.g. "sriya"
      if (s.id) aliases.add(s.id);                   // e.g. "STF-9877"
      if (s.uniqueId) aliases.add(s.uniqueId);       // e.g. "STF-VMFZUI"
      if (s.staffId) aliases.add(s.staffId);
      if (s.phone) aliases.add(s.phone.replace(/\D/g, ''));
      // The logged-in staff's uniqueId always maps to currentStaff key
      if (key === currentStaff && loggedInUniqueId) aliases.add(loggedInUniqueId);
      staffIdAliases[key] = aliases;
    });

    // Helper: does a booking's staffId array contain any alias for this STAFF_DATA key?
    function bookingBelongsToKey(booking, key) {
      const bookingStaffIds = Array.isArray(booking.staffId)
        ? booking.staffId
        : (booking.staffId ? [booking.staffId] : []);
      const aliases = staffIdAliases[key];
      return bookingStaffIds.some(bsid => aliases.has(bsid));
    }

    // Map bookings to staff appointment objects
    const mapBooking = b => {
      let serviceNames = [];
      let totalDuration = 0;
      if (b.services && Array.isArray(b.services)) {
        b.services.forEach(sid => {
          const svc = serviceCatalog.find(c => String(c.id) === String(sid));
          if (svc) {
      ...
      const aptDate = (b.date && b.date.includes('T')) ? b.date.split('T')[0] : (b.date || '');
      return {
        id: b.id || b._id,
    ...
    Object.keys(STAFF_DATA).forEach(staffKey => {
      if (!STAFF_DATA[staffKey].appointments) {
        STAFF_DATA[staffKey].appointments = [];
      }
      const staffApts = STAFF_DATA[staffKey].appointments;

      // Only today's bookings assigned to this staff member
      const remoteApts = bookings
        .filter(b => {
          const aptDate = (b.date && b.date.includes('T')) ? b.date.split('T')[0] : (b.date || '');
          return aptDate === todayStr && bookingBelongsToKey(b, staffKey);
        })
        .map(mapBooking);
      ...
      // Add or update
      remoteApts.forEach(ra => {
        const localIdx = staffApts.findIndex(la => la.id === ra.id);
        if (localIdx === -1) {
          staffApts.push(ra);
          if (!aptStates[ra.id]) aptStates[ra.id] = ra.status;
          changed = true;
        } else if (JSON.stringify(staffApts[localIdx]) !== JSON.stringify(ra)) {
          staffApts[localIdx] = ra;
          changed = true;
        }
      });

      // Remove local appointments no longer in remote (deleted/cancelled/reassigned)
      for (let i = staffApts.length - 1; i >= 0; i--) {
        const la = staffApts[i];
        if ((!la.date || la.date === todayStr) && !remoteApts.some(ra => ra.id === la.id)) {
          staffApts.splice(i, 1);
          changed = true;
        }
      }

      if (changed) {
        staffApts.sort((a, b) => (a.time || '').localeCompare(b.time || ''));
        if (staffKey === currentStaff) {
          refreshTimeline();
        }
      }
    });
  } catch (error) {
    console.error("Unable to sync appointments", error);
  }
}
...
function loadStaff(id) {
  currentStaff = id;
  sessionStorage.setItem('loggedInStaffId', id);
  ...
  let displayName = sessionStorage.getItem('loggedInUser') || s.name;
  ...
  // Dynamic Role logic
  const registeredRole = sessionStorage.getItem('loggedInRole') || s.role || "Senior Stylist";
  ...
  if (sidebarRoleEl) {
    ...
    const loc = sessionStorage.getItem('lastLoginLocation');
    const time = sessionStorage.getItem('lastLoginTime');
  ...
  const registeredPhone = sessionStorage.getItem('loggedInPhone') || s.phone || "No phone added";
  const registeredEmail = sessionStorage.getItem('loggedInEmail') || s.email || "No email added";
  ...
  try {
    const lsSpec = sessionStorage.getItem('loggedInSpecialties');
    if (lsSpec) {
...
function downloadPayslip(month, base, comm, net) {
  ...
  let displayName = sessionStorage.getItem('loggedInUser') || staff.name;
  if (displayName && displayName.includes('@')) {
    let namePart = displayName.split('@')[0];
...
window.selectedServiceToAdd = null;

window.renderAddServiceList = function(query) {
  const list = document.getElementById("addServiceList");
  if (!list) return;
  
  const q = (query || "").toLowerCase();
  const filtered = serviceCatalog.filter(s => s.name.toLowerCase().includes(q) || (s.cat && s.cat.toLowerCase().includes(q)));
  
  if (filtered.length === 0) {
     list.innerHTML = `<div style="padding:20px; text-align:center; color:var(--muted);">No services found.</div>`;
     return;
  }
  
  let html = "";
  filtered.forEach(s => {
     const isSelected = window.selectedServiceToAdd === s.id;
     const bg = isSelected ? 'var(--primary-light)' : 'var(--bg)';
     const border = isSelected ? 'var(--primary)' : 'var(--border)';
     html += `
       <div class="ripple" onclick="selectServiceToAdd('${s.id}')" style="padding: 12px; margin-bottom: 8px; border: 1px solid ${border}; border-radius: 8px; background: ${bg}; cursor: pointer; display: flex; justify-content: space-between; align-items: center; transition: all 0.2s;">
         <div>
           <div style="font-weight: 600; font-size: 14px; color: var(--fg); text-align: left;">${s.name}</div>
           <div style="font-size: 12px; color: var(--muted); margin-top: 4px; text-align: left;">⏱ ${s.duration} mins</div>
         </div>
         <div style="font-weight: 700; color: var(--primary);">₹${s.price.toLocaleString("en-IN")}</div>
       </div>
     `;
  });
  list.innerHTML = html;
...
window.selectServiceToAdd = function(id) {
  window.selectedServiceToAdd = id;
  const searchInput = document.getElementById("addServiceSearch");
  if (typeof window.renderAddServiceList === 'function') {
      window.renderAddServiceList(searchInput ? searchInput.value : "");
  }
};
...
window.confirmAddService = function() {
  if (!window.selectedServiceToAdd || !window.currentActiveApt) {
      if (typeof showToast === 'function') showToast("Please select a service first.", "warning", "⚠️");
      return;
  }
  const svcId = window.selectedServiceToAdd;
...
function buildTimeline(s, typeFilter = '') {
  ...
  if (outdoorHub) {
    if (typeFilter === 'outdoor') {
      ...
      outdoorHub.classList.add("visible");
      outdoorHub.style.display = 'block';
      ...
      outdoorHub.classList.remove("visible");
      outdoorHub.style.display = 'none';
    }
  }
  ...
  if (typeFilter) {
    appointments = appointments.filter(a => {
      ...
      if (!svc) {
        ...
        svc = serviceCatalog.find(sc => {
          ...
          return scName.includes(aName) || aName.includes(scName) || scCat.includes(aName) || aName.includes(scCat);
        });
      }
    ...
    if (subEl) subEl.innerHTML = `<span style="color:var(--primary);cursor:pointer;font-weight:700;" onclick="refreshTimeline()">✕ Clear ${typeFilter} filter</span>`;
  }
  ...
  if (appointments.length > 0) {
    ...
    appointments.forEach((a) => {
      ...
      // Insert "NOW" line before the first upcoming appointment that hasn't started yet
      if (!nowInserted && aTime >= nowStr && (aptStates[a.id] || a.status) === 'upcoming') {
...
function submitNewAppointment() {
  ...
  // Sync to MongoDB
  fetch(`http://localhost:3000/api/staff/${currentStaff}`, {
    method: 'PUT',
...
function startApt(id, e) {
  ...
  // Sync to Admin Backend (Two-way sync)
  fetch(`http://localhost:5000/api/bookings/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ status: 'in-progress' })
  }).catch(err => console.error("Failed to sync start status to admin:", err));
}
...
function doneApt(id, e) {
  ...
  fetch(`http://localhost:5000/api/bookings/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ status: 'Completed' }) // Backend uses 'Completed'
  }).catch(err => console.error("Failed to sync complete status to admin:", err));
}
...
async function buildLeave(s) {
  ...
  leaveListEl.innerHTML = '<div style="text-align:center;padding:20px;font-size:13px;color:var(--muted)">Loading...</div>';

  const staffId = sessionStorage.getItem('loggedInUniqueId') || sessionStorage.getItem('loggedInPhone') || currentStaff;

  try {
    const res = await fetch('http://localhost:5000/api/leave-requests');
    if (res.ok) {
      const allRequests = await res.json();
      // Filter by staffId - try multiple fields for robustness
      const myLeaves = allRequests.filter(r => 
        r.staffId === staffId || 
        r.staffId === (s && s.staffId) ||
        r.staffId === (s && s.id)
      );
      
      // Always override local cache with server data
      staffLeaves = myLeaves.map(r => ({
        id: r._id || r.id,
        type: r.reason ? (r.reason.toLowerCase().includes('sick') ? 'sick' : r.reason.toLowerCase().includes('casual') ? 'casual' : 'emergency') : 'casual',
        from: r.fromDate || r.startDate,
        to: r.toDate || r.endDate,
        reason: r.reason,
    ...
    staffLeaves = s.leaves || [];
    saveStaffLeaves();
  }

  if (!staffLeaves) staffLeaves = [];
    ...
    ? staffLeaves
      .map(
        (l) => {
          ...
          const statusColor = statusClass === 'approved' ? '#16a34a' : statusClass === 'rejected' ? '#dc2626' : '#d97706';
          const statusBg = statusClass === 'approved' ? '#dcfce7' : statusClass === 'rejected' ? '#fee2e2' : '#fef3c7';
          return `
      <div class="leave-item">
        ...
        <div class="leave-status" style="background:${statusBg};color:${statusColor};padding:4px 10px;border-radius:6px;font-size:12px;font-weight:700;">${statusLabel}</div>
      </div>`;
        }
      )
      .join("")
...
async function submitLeave() {
  ...
  try {
    const res = await fetch(`http://localhost:5000/api/staff/${staffId}/leave-request`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ startDate: from, endDate: to, reason: reason })
    });
    if (!res.ok) throw new Error('API failed');
  } catch (err) {
    console.error('Failed to sync leave request to backend:', err);
  }
...
function logout() {
  ...
  sessionStorage.removeItem('loggedInSpecialties');
...
function buildClients(s) {
  ...
  // Simple deduplication based on phone (if exists) or name
  const deduped = [];
  const seen = new Set();
  merged.forEach(c => {
  ...
  window.currentStaffClients = deduped;
  renderClientList(deduped);
}
...
function renderClientList(clients) {
  ...
  if (clients.length === 0) {
    clientListEl.innerHTML = '<div style="padding: 24px; text-align: center; color: var(--muted);">No clients found.</div>';
    return;
  }

  clientListEl.innerHTML = clients.map(c => {
    const initials = (c.name || 'U').split(' ').slice(0, 2).map(n => n[0]).join('').toUpperCase();
    const isMine = c.isEnriched;
    const isActive = window.currentClientId === c.id;
    
    return `
    <div id="client-item-${c.id}" class="client-item ripple" onclick="showClientDetails('${c.id}')" style="padding: 16px 20px; border-bottom: 1px solid var(--border); cursor: pointer; display: flex; align-items: center; gap: 16px; transition: all 0.2s; ${isActive ? 'background: var(--primary-light); border-left: 4px solid var(--primary);' : 'border-left: 4px solid transparent;'}">
      <div style="width: 44px; height: 44px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; font-weight: 600; flex-shrink: 0; background: ${isMine ? 'linear-gradient(135deg, var(--primary), #2b8cb1)' : 'var(--muted-bg)'}; color: ${isMine ? '#fff' : 'var(--muted)'}; box-shadow: ${isMine ? '0 4px 10px rgba(26,107,138,0.2)' : 'none'};">
        ${initials}
      </div>
      <div style="flex: 1; min-width: 0;">
        <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 4px;">
          <div style="font-weight: 600; color: ${isActive ? 'var(--primary)' : 'var(--fg)'}; font-size: 15px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: flex; align-items: center; gap: 8px;">
            ${c.name}
            ${c.totalVisits > 10 ? `<span style="background: linear-gradient(135deg, #f59e0b, #d97706); color: white; padding: 2px 8px; border-radius: 12px; font-size: 10px; font-weight: 700;">VIP</span>` : ''}
          </div>
          ${isMine ? `<div style="width: 8px; height: 8px; border-radius: 50%; background: var(--primary); flex-shrink: 0;" title="Your Client"></div>` : ''}
        </div>
        <div style="font-size: 12px; color: var(--muted); display: flex; align-items: center; gap: 6px;">
          <span>🕒</span>
          <span style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${c.lastVisit === "No recent visit" ? 'New Client' : c.lastVisit}</span>
        </div>
      </div>
    </div>
  `}).join("");
...
function showClientDetails(clientId) {
  ...
  if (window.currentStaffClients) {
    const query = document.getElementById("clientSearch")?.value.toLowerCase() || "";
    const filtered = window.currentStaffClients.filter(c => c.name.toLowerCase().includes(query) || (c.phone && c.phone.includes(query)));
    renderClientList(filtered);
  }
  ...
  detailEl.style.minHeight = "calc(100vh - 200px)";
  detailEl.style.height = "auto";
  detailEl.style.padding = "0";
  detailEl.style.textAlign = "left";
  detailEl.style.alignItems = "stretch";
  detailEl.style.justifyContent = "flex-start";

  const initials = (client.name || 'U').split(' ').slice(0, 2).map(n => n[0]).join('').toUpperCase();

  const formulasHtml = client.formulas && client.formulas.length > 0 ? client.formulas.map(f => `
    <div style="background: var(--card); border: 1px solid var(--border); padding: 16px; border-radius: 12px; margin-bottom: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
        <span style="font-weight: 600; font-size: 14px; color: var(--fg); display: flex; align-items: center; gap: 6px;"><span style="color: var(--primary);">✨</span> ${f.service}</span>
        <span style="color: var(--muted); font-size: 12px; background: var(--muted-bg); padding: 4px 8px; border-radius: 6px;">${f.date}</span>
      </div>
      <div style="font-size: 13.5px; color: var(--fg); background: var(--primary-light); padding: 12px; border-radius: 8px; border-left: 3px solid var(--primary); line-height: 1.5;">
        ${f.formula}
      </div>
    </div>
  `).join("") : '<div style="color: var(--muted); font-size: 14px; padding: 20px; background: var(--muted-bg); border-radius: 12px; border: 1px dashed var(--border); text-align: center;">No specific formulas recorded yet.</div>';

  detailEl.innerHTML = `
    <div style="padding: 32px 32px 24px; border-bottom: 1px solid var(--border); background: linear-gradient(to bottom, var(--primary-light) 0%, var(--card) 100%); border-radius: var(--radius) var(--radius) 0 0;">
      <div style="display: flex; justify-content: space-between; align-items: flex-start;">
        <div style="display: flex; align-items: center; gap: 20px;">
          <div style="width: 72px; height: 72px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: 700; background: linear-gradient(135deg, var(--primary), #2b8cb1); color: #fff; box-shadow: 0 6px 16px rgba(26,107,138,0.25);">
            ${initials}
          </div>
          <div>
            <h2 style="margin: 0 0 6px 0; font-size: 24px; color: var(--fg); font-family: 'Playfair Display', serif; display: flex; align-items: center; gap: 12px;">
              ${client.name}
              ${client.totalVisits > 10 ? `<span style="background: linear-gradient(135deg, #f59e0b, #d97706); color: white; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; font-family: 'Inter', sans-serif; text-transform: uppercase; letter-spacing: 0.5px; box-shadow: 0 4px 12px rgba(245,158,11,0.25);">👑 VIP Client</span>` : ''}
            </h2>
            <div style="display: flex; align-items: center; gap: 12px; color: var(--muted); font-size: 14px;">
              <span style="display: flex; align-items: center; gap: 4px;">📞 ${client.phone || 'N/A'}</span>
              ${client.totalVisits ? `<span style="width: 4px; height: 4px; border-radius: 50%; background: var(--muted);"></span><span style="font-weight: 500;">${client.totalVisits} Visits</span>` : ''}
            </div>
          </div>
        </div>
        <button class="btn-primary btn-sm ripple" style="border-radius: 8px; padding: 8px 16px; font-weight: 600;" onclick="showToast('Edit feature coming soon!', 'info', 'ℹ')">
          ✎ Edit
        </button>
      </div>
    </div>
    ...
    <div style="padding: 24px 32px;">
      <div style="margin-bottom: 32px;">
        <h3 style="font-size: 13px; text-transform: uppercase; letter-spacing: 1.2px; color: var(--muted); font-weight: 700; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
          <span>📝</span> Client Notes
        </h3>
        <div style="background: var(--card); padding: 16px; border-radius: 12px; border: 1px solid var(--border); font-size: 14.5px; line-height: 1.6; color: var(--fg); box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
          ${client.notes || '<span style="color: var(--muted); font-style: italic;">No specific notes recorded with you.</span>'}
        </div>
      </div>

      <div>
        <h3 style="font-size: 13px; text-transform: uppercase; letter-spacing: 1.2px; color: var(--muted); font-weight: 700; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
          <span>🔬</span> Treatment History & Formulas
        </h3>
...
async function submitInventoryRequest() {
  ...
  const staffName = (typeof STAFF_DATA !== 'undefined' && STAFF_DATA[currentStaff]) ? STAFF_DATA[currentStaff].name : (sessionStorage.getItem('loggedInUser') || "Staff Member");

  try {
    // Save request to localStorage so Admin portal can see it
    const existingRequests = JSON.parse(localStorage.getItem('openStockRequests') || '[]');
    const newRequest = {
      id: 'req-' + Date.now(),
      item: item,
      qty: Number(qty),
    ...
    localStorage.setItem('openStockRequests', JSON.stringify(existingRequests));

    // Attempt to sync to backend (might not exist yet), but do not fail if it errors
    fetch("http://localhost:3000/api/inventory-requests", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ item, quantity: Number(qty), urgency, station, staffName })
    }).catch(err => console.warn("Inventory API not ready:", err));
    ...
    setTimeout(() => {
      ...
      if (staffChat) {
        ...
        body.innerHTML += `<div class="chat-msg received" style="animation:fadeUp 0.2s backwards;">We received your request for ${qty}x ${item}. It's being prepared for ${station}!${urgentText}</div>`;
        body.scrollTop = body.scrollHeight;
      }
    }, 2500);
...
function renderIndoorModalContent() {
  ...
  if (filteredServices.length > 0) {
    filteredServices.forEach(s => {
      ...
      let clickHandler = '';
      if (window.currentServiceModalType === 'walkin') {
        const safeName = s.name.replace(/'/g, "\\'");
        clickHandler = `onclick="openWalkinDetailsModal('${safeName}', ${s.price}, '${s.duration}')" style="cursor: pointer;"`;
      } else if (window.currentServiceModalType === 'preschedule') {
        const safeName = s.name.replace(/'/g, "\\'");
        clickHandler = `onclick="openPrescheduleModal('${safeName}', ${s.price}, '${s.duration}')" style="cursor: pointer;"`;
      }

      svcsHtml += `
        <div class="cs-row indoor-service-card ripple" ${clickHandler} style="min-height: 125px; background: var(--bg); padding: 16px; border-radius: 12px; border: 1px solid var(--border); display: flex; flex-direction: column;">
          ${badgeHtml}
...
function openServiceTypeModal(type) {
  ...
  const rightPanel = splitWrapper.children[1];
  const leftPanel = splitWrapper.children[0];
  const searchWrapper = document.querySelector('.indoor-search-wrapper');
  const tabsContainer = document.getElementById('indoorCategoryTabs');
  const servicesGrid = document.getElementById('stServicesGrid');
  ...
  if (type === 'walkin') {
    ...
    leftPanel.style.borderRight = '1px solid var(--border)';
    leftPanel.style.paddingRight = '24px';
    servicesGrid.style.gridTemplateColumns = 'repeat(auto-fill, minmax(220px, 1fr))';
  } else {
...
// --- WALKIN SERVICES LOGIC ---
let pendingWalkinService = null;

function openWalkinDetailsModal(serviceName, price, duration) {
  pendingWalkinService = { serviceName, price, duration };
  
  document.getElementById('walkinServiceName').textContent = serviceName;
  document.getElementById('walkinServicePrice').textContent = `₹${price.toLocaleString("en-IN")}`;
  document.getElementById('walkinServiceDuration').textContent = `${duration} mins`;
  
  document.getElementById('walkinClientName').value = '';
  document.getElementById('walkinClientPhone').value = '';
  
  document.getElementById('walkinDetailsModalOverlay').classList.remove('hidden');
}

function closeWalkinDetailsModal() {
  document.getElementById('walkinDetailsModalOverlay').classList.add('hidden');
  pendingWalkinService = null;
}

function submitWalkinAppointment() {
  const clientName = document.getElementById('walkinClientName').value.trim();
  const clientPhone = document.getElementById('walkinClientPhone').value.trim();
  
  if (!clientName) {
    showToast("Please enter a Client Name", "warning", "⚠️");
    return;
  }
  ...
  hh = hh % 12 || 12;
  const timeStr = `${hh}:${mm.toString().padStart(2, '0')} ${ampm}`;
  
  s.appointments.push({
    id: newId,
    time: timeStr,
    client: clientName,
    service: pendingWalkinService.serviceName,
    duration: pendingWalkinService.duration,
    price: pendingWalkinService.price,
    status: "in-progress",
  ...
  if (typeof buildDashboard === 'function') buildDashboard(s);
  if (typeof buildTimeline === 'function') buildTimeline(s);
  if (typeof buildStats === 'function') buildStats(s); // Update top stats grid!
  showToast("Walk-in appointment started!", "success", "🚶");
}

// --- PRESCHEDULE SERVICES LOGIC ---
let pendingPrescheduleService = null;

function openPrescheduleModal(serviceName, price, duration) {
  pendingPrescheduleService = { serviceName, price, duration };
  
  document.getElementById('prescheduleServiceName').textContent = serviceName;
  document.getElementById('prescheduleServicePrice').textContent = `₹${price.toLocaleString("en-IN")}`;
  document.getElementById('prescheduleServiceDuration').textContent = `${duration} mins`;
  
  document.getElementById('prescheduleClientName').value = '';
  document.getElementById('prescheduleClientPhone').value = '';
  document.getElementById('prescheduleDate').value = '';
  document.getElementById('prescheduleTime').value = '';
  
  document.getElementById('prescheduleDetailsModalOverlay').classList.remove('hidden');
}

function closePrescheduleModal() {
  document.getElementById('prescheduleDetailsModalOverlay').classList.add('hidden');
  pendingPrescheduleService = null;
}

function submitPrescheduleAppointment() {
  const clientName = document.getElementById('prescheduleClientName').value.trim();
  const clientPhone = document.getElementById('prescheduleClientPhone').value.trim();
  const dateStr = document.getElementById('prescheduleDate').value;
  let timeStr = document.getElementById('prescheduleTime').value;
  
  if (!dateStr || !timeStr || !clientName) {
    showToast("Please fill Date, Time, and Client Name", "warning", "⚠️");
    return;
  }
  
  // Format timeStr from 24h to 12h
  let [hh, mm] = timeStr.split(':');
  hh = parseInt(hh, 10);
  const ampm = hh >= 12 ? 'PM' : 'AM';
  hh = hh % 12 || 12;
  timeStr = `${hh}:${mm.toString().padStart(2, '0')} ${ampm}`;
  ...
  if (dateStr === today) {
    s.appointments.push({
      ...
      time: timeStr,
      client: clientName,
      service: pendingPrescheduleService.serviceName,
      duration: pendingPrescheduleService.duration,
      price: pendingPrescheduleService.price,
      status: "upcoming",
      phone: clientPhone || "N/A"
    });
    ...
    if (!window.allBookings) window.allBookings = [];
  ...
  closePrescheduleModal();
  closeServiceTypeModal();
  
  if (typeof buildDashboard === 'function') buildDashboard(s);
  if (typeof buildTimeline === 'function') buildTimeline(s);
  if (typeof buildStats === 'function') buildStats(s);
  
  showToast(`Appointment scheduled for ${clientName}!`, "success", "📅");
}
...
// ── INVENTORY UPDATES LISTENER ───────────────────────────────────────────────
window.addEventListener('storage', function(e) {
  if (e.key === 'latestStockAction' && e.newValue) {
    try {
      const actionData = JSON.parse(e.newValue);
      if (actionData && actionData.action && actionData.item) {
        const isApproved = actionData.action === 'Approved';
        const icon = isApproved ? "✅" : "❌";
        const theme = isApproved ? "success" : "danger";
        
        if (typeof showToast === 'function') {
          showToast(`Your request for ${actionData.qty}x ${actionData.item} was ${actionData.action}`, theme, icon);
        }
        ...
        if (staffChat) {
          ...
          const body = document.getElementById("chatBody");
          if (body) {
            body.innerHTML += `<div class="chat-msg received" style="animation:fadeUp 0.2s backwards;">Admin has <b>${actionData.action.toUpperCase()}</b> your request for ${actionData.qty}x ${actionData.item} for ${actionData.station}.</div>`;
            body.scrollTop = body.scrollHeight;
          }
        }
      }
